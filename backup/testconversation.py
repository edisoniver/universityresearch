...
def temperature(update: Update, _: CallbackContext) -> int:
    user = update.message.from_user
    Keyboard = [
        [
...
def output(update: Update, _: CallbackContext):
    ...
    #    enginebtn = 'ada'
    user = update.message.from_user
    logger.info("ENGINE: %s", str(update.message.text))
    global engine_value
    ...
    update.message.reply_text("Prompt: {0}\n\nEngine: {1}".format(prompt_value, engine_value))
    call() #call the OPENAI API HERE 
    for x in textoutput3:
        update.message.reply_text(x)
    update.message.reply_text("/start input a prompt again")
